...
func main() {
	...
	for {
		// select statements with double conditions for each channel
		// when receiving data from a channel, select chose
		// randomly which case with the same condition will be executed
		select {
		case s1 := <-channel1:
